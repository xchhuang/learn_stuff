...
class Dither():
    ...
    def apply_threshold(self, value):
        ...
        return 255 * floor(value/128)   # 255 * floor(value/128)

    def floyd_steinberg_dither(self, image_file):
        # print('floyd_steinberg_dither')
        """
        ...
        # pixel_ori = np.asarray(Image.open(image_file).convert('RGB')) / 255.0
        pixel_new = np.asarray(Image.open(self.output).convert('RGB')) / 255.0
...
def main():
    ...
    dither = Dither(args.image_path, output=args.o)

    path = args.image_path
    for i in tqdm(range(1)):
        print(path)
        pixel = dither.floyd_steinberg_dither(path)
        path = args.o
